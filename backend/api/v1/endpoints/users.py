...
async def get_session_trace(
    ...
    for run in runs:
        ...
        attempts = db.query(RAGRunAttemptTable).filter(RAGRunAttemptTable.request_id == run.request_id).order_by(RAGRunAttemptTable.attempt_no.asc()).all()
        events = db.query(RAGEventTable).filter(RAGEventTable.request_id == run.request_id).order_by(RAGEventTable.attempt_no.asc(), RAGEventTable.sequence_no.asc()).all()
        manifests = db.query(ContextManifestTable).filter(ContextManifestTable.request_id == run.request_id).order_by(ContextManifestTable.attempt_no.asc(), ContextManifestTable.iteration.asc()).all()
        payload.append({"id": run.id, "request_id": run.request_id, "user_id": run.user_id, "session_id": run.session_id, "kb_id": run.kb_id, "route_type": run.route_type, "final_status": run.final_status, "answer_mode": run.answer_mode, "reflection_count": run.reflection_count, "total_latency_ms": run.total_latency_ms, "input_tokens": run.input_tokens, "output_tokens": run.output_tokens, "selected_evidence_ids": run.selected_evidence_ids or [], "selected_memory_ids": run.selected_memory_ids or [], "runtime_mode": run.runtime_mode or "react", "iteration_count": int(run.iteration_count or 0), "stop_reason": run.stop_reason, "tool_call_count": int(run.tool_call_count or 0), "budget_profile": run.budget_profile, "shadow_metrics": run.shadow_metrics or {}, "error": run.error, "created_at": run.created_at.isoformat() if run.created_at else None, "completed_at": run.completed_at.isoformat() if run.completed_at else None, "spans": [{"id": span.id, "request_id": span.request_id, "attempt_no": int(span.attempt_no or 1), "span_name": span.span_name, "node_name": span.node_name, "span_kind": span.span_kind or "agent_node", "status": span.status, "started_at": span.started_at.isoformat() if span.started_at else None, "ended_at": span.ended_at.isoformat() if span.ended_at else None, "latency_ms": span.latency_ms, "model_name": span.model_name, "input_tokens": span.input_tokens, "output_tokens": span.output_tokens, "metadata": span.metadata_json or {}, "error": span.error} for span in spans], "attempts": [{"id": item.id, "request_id": item.request_id, "attempt_no": item.attempt_no, "status": item.status, "resumed": item.resumed, "stop_reason": item.stop_reason, "error": item.error, "started_at": item.started_at.isoformat() if item.started_at else None, "ended_at": item.ended_at.isoformat() if item.ended_at else None} for item in attempts], "events": [{"id": item.id, "request_id": item.request_id, "attempt_no": item.attempt_no, "sequence_no": item.sequence_no, "event_name": item.event_name, "node_name": item.node_name, "status": item.status, "metadata": item.metadata_json or {}, "created_at": item.created_at.isoformat() if item.created_at else None} for item in events], "context_manifests": [{"id": item.id, "request_id": item.request_id, "attempt_no": item.attempt_no, "node_name": item.node_name, "iteration": item.iteration, "model_name": item.model_name, "token_budget": item.token_budget, "token_used": item.token_used, "loaded_sections": item.loaded_sections or [], "omitted_sections": item.omitted_sections or [], "section_usage": item.section_usage or {}, "created_at": item.created_at.isoformat() if item.created_at else None} for item in manifests]})
