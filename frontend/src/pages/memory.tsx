...
export function MemoryPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const result = await memoryService.list({ status: status === "all" ? undefined : status, memory_category: memoryCategory === "all" ? undefined : memoryCategory, memory_type: memoryType === "all" ? undefined : memoryType, scope_type: scopeType === "all" ? undefined : scopeType, page_size: 100 })
      setItems(result.items || [])
    } catch { setNotice({ text: "记忆加载失败，请稍后重试。", error: true }) } finally { setLoading(false) }
  }
  useEffect(() => { void load() }, [status, memoryCategory, memoryType, scopeType])
  ...
  return <div className="min-h-screen p-5 lg:p-8"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <div className="flex flex-wrap gap-2"><Select className="w-40" value={memoryCategory} onChange={(event) => setMemoryCategory(event.target.value)}><option value="all">全部认知分类</option><option value="episodic">情景记忆</option><option value="semantic">语义记忆</option><option value="procedural">程序记忆</option></Select><Select className="w-44" value={memoryType} onChange={(event) => setMemoryType(event.target.value)}><option value="all">全部业务类型</option><option value="user_profile">用户事实</option><option value="user_preference">用户偏好</option><option value="project_context">项目背景</option><option value="project_decision">项目决策</option><option value="project_constraint">项目约束</option><option value="episodic_event">情景事件</option><option value="workflow">工作流程</option></Select><Select className="w-36" value={scopeType} onChange={(event) => setScopeType(event.target.value)}><option value="all">全部作用域</option><option value="user">user</option><option value="project">project</option></Select><Select className="w-44" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">全部状态</option><option value="active">active</option><option value="pending_confirmation">待确认</option><option value="rejected">rejected</option><option value="inactive">inactive</option></Select></div>
