...
上级导航：[文档中心](../README.md) · 当前文档：长期工程目标与演进路线

当前 ReAct 实现见[系统架构与运行流程](../02-系统架构与运行流程.md)，工具协议见[工具系统与检索](../04-工具系统与检索.md)，迁移标准见[数据库迁移与 ReAct 上线验收](../06-数据库迁移与ReAct上线验收.md)。本文件只作为长期能力蓝图。
